...
def game_a(num_of_rounds):
    """Standard difficulty of game."""
    print(f'Play {num_of_rounds} rounds of regular game')
    while len(round_counter) < num_of_rounds:
        options = 'r', 'p', 's'
...
def stats_print():
    ...
    for elem in result:
        if elem == 'd':
            ...
            comp_wins += 1
    user_wins_percent = user_wins / len(round_counter) * 100
...
def game_end():
    ...
    if user_decision == 'n' and len(round_counter) == 0:
        print(f'Game Over')
